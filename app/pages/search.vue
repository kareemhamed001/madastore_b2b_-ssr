<template>
  <div class="mx-auto w-full max-w-7xl px-4 py-8">
    <header class="mb-8">
      <h1 class="mb-6 text-3xl font-bold text-[var(--mada-dark)] md:text-4xl">
        {{ t('search.title') }}
      </h1>
    ...
    <div
      v-if="searchQuery"
      class="mb-8 rounded-md bg-[var(--mada-soft)] px-4 py-4 text-[var(--mada-dark)]"
    >
      <p>
    ...
    <div
      v-if="loading"
      class="px-4 py-12 text-center text-[var(--mada-muted)]"
    >
    ...
    <div
      v-else-if="error"
      class="px-4 py-12 text-center text-red-600"
    >
      <p>Error: {{ error }}</p>
      <button
    ...
    <div v-else-if="hasSearchTerm">
      <div v-if="categoryResults.length > 0" class="mb-12">
        <h2 class="mb-6 text-2xl font-semibold text-[var(--mada-dark)]">
          {{ t('search.categoriesCount', { count: categoryResults.length }) }}
        </h2>
        ...
        <div class="mb-6 flex items-start justify-between gap-3">
          <div>
            ...
            <p class="text-sm text-[var(--mada-muted)]">
              {{ t('search.productsCount', { count: productStore.pagination.total }) }}
            </p>
          </div>
          <button
            type="button"
        ...
        <div class="flex flex-col gap-6 lg:flex-row lg:items-start">
          <aside class="hidden w-full lg:sticky lg:top-24 lg:block lg:w-72 lg:flex-none">
            <ProductFiltersSidebar
              ...
              :filter-options="productStore.filterOptions"
              :loading-options="productStore.filterOptionsLoading"
          ...
          <div class="min-w-0 flex-1">
            ...
            <ProductList
              :products="productStore.searchResults"
      ...
      <div
        v-if="categoryResults.length === 0 && productStore.searchResults.length === 0"
        class="px-4 py-12 text-center text-[var(--mada-muted)]"
      >
        <p>{{ t('search.noResultsFor', { query: searchQuery || activeFilters.name }) }}</p>
    ...
    <div v-if="mobileFiltersOpen" class="fixed inset-0 z-40 lg:hidden">
      ...
      />
      <div
        class="absolute bottom-0 top-0 w-full max-w-[22rem] overflow-y-auto bg-[var(--mada-surface)] p-4 shadow-xl"
...
const hasSearchTerm = computed(() => {
  return Boolean(searchQuery.value || activeFilters.value.name)
})
...
const emptyProductsMessage = computed(() => {
  return hasAnyFilter.value ? t('category.noProductsForFilters') : t('search.noProductsMatch')
})
...
function serializeFiltersToQuery(filters, queryText) {
  const query = {}
...
function composeSearchName(primaryQuery, filterName) {
  return [primaryQuery, filterName]
    .map((value) => String(value || '').trim())
    .filter(Boolean)
    .join(' ')
    .trim()
...
function buildSearchParams(filters, options = {}) {
  ...
  const params = {}
  const finalName = composeSearchName(searchQuery.value, filters.name)
  ...
  if (filters.categories.length > 0) {
    ...
    params.categories_ids = filters.categories.join(',')
  }
  if (filters.sellers.length > 0) {
...
function pushFilters(nextFilters, nextQuery = searchQuery.value) {
  router.push({
    name: 'search',
    query: serializeFiltersToQuery(nextFilters, nextQuery),
  })
}
...
function handleSearch(query) {
  ...
  pushFilters(normalized, query.trim())
}
...
async function performSearch() {
  ...
  try {
    ...
    const categoryNeedle = searchQuery.value || filters.name

    productStore.setPage(filters.page)

    const [categories] = await Promise.all([
      categoryStore.searchCategories(categoryNeedle),
      productStore.searchProducts(searchParams),
      productStore.fetchFilterOptions(filterParams),
    ])

    categoryResults.value = categories
  } catch (err) {
    error.value = err.message || 'Unable to load search results.'
